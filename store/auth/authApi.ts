...
const getAccessTokenFromResponse = (payload: unknown): string | undefined => {
  ...
  return (
    d?.data?.user?.access_token ?? d?.data?.access_token ?? d?.access_token
  );
};
...
export const needsOtpVerificationBeforeSession = (
  ...
  if (p?.data && typeof p.data === "object" && "is_verified" in p.data) {
    ...
    getProfile: builder.query<any, void>({
      ...
      transformResponse: (response: any) => {
        ...
        if (
          ...
          return { ...(data as object), message: topMessage };
        }
